...
const createClientPaymentIntent = async (req, res) => {
  try {
    const { orderId, lawyerId } = req.body;
    ...
    const result = await paymentService.createPaymentIntentForClient({
      orderId,
      lawyerId,
    });
    ...
    return res
      .status(200)
      .json({ success: true, message: "Client secret generated", result });
  } catch (err) {
